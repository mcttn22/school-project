...
class TestDatabase(unittest.TestCase):
    ...
    def test_not_null_constraint(self) -> None:
        """Test that the NOT NULL constraint is setup."""
        connection = sqlite3.connect(database='school_project/saved_models.db')
        cursor = connection.cursor()

        # Try to insert record with the last attribute missing
    ...
    def test_save_load_consistency(self) -> None:
        """Test that data is not changed between saving and loading."""
        connection = sqlite3.connect(database='school_project/saved_models.db')
        cursor = connection.cursor()
