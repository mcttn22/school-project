...
The classes utilised to implement the UI are shown in Figure \ref{fig:UI-class-diagram}. As can be seen the School\_Project\_Frame inherits the tk.Frame class - a tkinter base 
...
The detail of algorithms underpinning the Artifical Neural Network are outlined in section \ref{sec:ann-theory}. To implement the algorithm each layer within the Artificial 
Neural Network is represented as an instance of a \textit{FullyConnectedLayer} class each of which:
...
I will also give the option to load pretrained weights to run on less computationally powerful hardware using just the CPU as standard. This was implemented by using the CuPy 
library.
